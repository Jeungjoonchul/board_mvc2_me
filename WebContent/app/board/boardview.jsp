	...
	<div id="wrap">
		...
		<form>
			...
			<table border="1" style="border-collapse: collapse;"
				...
				<tr>
					<th>작성일</th>
					<td>${board.regdate}<c:if
							test="${board.regdate != board.updatedate}">(수정됨 : <span>${board.updatedate}</span>)</c:if></td>
				</tr>
				<tr>
					<th>조회수</th>
		...
		<table class="btn_area">
			<tr align="right" valign="middle">
				...
				<c:if test="${board.userid==loginUser}">
					<a href="${cp }/board/boardmodify.bo?boardnum=${board.boardnum}&page=${param.page}">수정</a>&nbsp;&nbsp;
					<a href="${cp }/board/boardremove.bo?boardnum=${board.boardnum}">삭제</a>&nbsp;&nbsp;
				</c:if> 
					<a href="${cp }/board/boardlist.bo?page=${param.page}">목록</a></td>
			</tr>
		</table>
		<div class="reply_line">
			<form name="replyForm" method="post" action="">
				<table class="write_box">
					<tr>
						<td>댓글</td>
						<td><textarea name="replycontents"></textarea> <a href="#">등록</a>
						</td>
					</tr>
				</table>
			</form>
			<form name="updateForm">
				<table class="update_box">
					<tr>
						<td>댓글 작성자</td>
						<td><textarea readonly>댓글내용</textarea></td>
						<td>작성시간<br>
							<div class="btns">
								<a href="">수정</a> <a href="" style="display: none;">수정완료</a> <a
									href="">삭제</a>
							</div>
						</td>
					</tr>
